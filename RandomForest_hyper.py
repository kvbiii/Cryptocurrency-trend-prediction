...
class RandomForest_tuning():
    ...
    def hyper_optuna(self):
        if(self.estimator == self.nazwa_estymatora):
            ...
            criterion_list = study_df.loc[:study_df.index[10], "params_criterion"].values
            max_depth_list = study_df.loc[:study_df.index[10], "params_max_depth"].values
            min_samples_split_list = study_df.loc[:study_df.index[10], "params_min_samples_split"].values
            min_samples_leaf_list = study_df.loc[:study_df.index[10], "params_min_samples_leaf"].values
            i = 0
            best_mean_score = 0
            while(i < len(criterion_list)):
                scores = []
                for kwantyl in range(1, len(self.kwantyle)+1):
                    train = self.train_lista[kwantyl-1]
                    valid = self.valid_lista[kwantyl-1]
                    ...
                    for fold in range(1, self.number_of_folds+1):
                        ...
                        model = RandomForestClassifier(criterion=criterion_list[i], max_depth=max_depth_list[i], min_samples_split=min_samples_split_list[i], min_samples_leaf=min_samples_leaf_list[i], random_state=17)
            ...
            for kwantyl in range(1, len(self.kwantyle)+1):
                ...
                for fold in range(1, self.number_of_folds+1):
                    ...
                    estimator_final = RandomForestClassifier(criterion=criterion_list[best_set], max_depth=max_depth_list[best_set], min_samples_split=min_samples_split_list[best_set], min_samples_leaf=min_samples_leaf_list[best_set], random_state=17)
