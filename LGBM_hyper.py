...
class LGBM_tuning():
    ...
    def hyper_optuna(self):
        if(self.estimator == self.nazwa_estymatora):
            ...
            num_leaves_list = study_df.loc[:study_df.index[10], "params_num_leaves"].values
            learning_rate_list = study_df.loc[:study_df.index[10], "params_learning_rate"].values
            colsample_bytree_list = study_df.loc[:study_df.index[10], "params_colsample_bytree"].values
            subsample_list = study_df.loc[:study_df.index[10], "params_subsample"].values
            min_child_samples_list = study_df.loc[:study_df.index[10], "params_min_child_samples"].values
            i = 0
            best_mean_score = 0
            while(i < len(num_leaves_list)):
                scores = []
                for kwantyl in range(1, len(self.kwantyle)+1):
                    train = self.train_lista[kwantyl-1]
                    valid = self.valid_lista[kwantyl-1]
                    ...
                    for fold in range(1, self.number_of_folds+1):
                        ...
                        model = LGBMClassifier(boosting_type="gbdt", num_leaves=num_leaves_list[i], learning_rate=learning_rate_list[i], colsample_bytree=colsample_bytree_list[i], subsample=subsample_list[i], min_child_samples=min_child_samples_list[i], objective="binary", random_state=17)
            ...
            for kwantyl in range(1, len(self.kwantyle)+1):
                ...
                for fold in range(1, self.number_of_folds+1):
                    ...
                    estimator_final = LGBMClassifier(boosting_type="gbdt", num_leaves=num_leaves_list[best_set], learning_rate=learning_rate_list[best_set], colsample_bytree=colsample_bytree_list[best_set], subsample=subsample_list[best_set], min_child_samples=min_child_samples_list[best_set], objective="binary", random_state=17)
