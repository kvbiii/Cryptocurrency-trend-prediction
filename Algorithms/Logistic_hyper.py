...
class Logistic_tuning():
    ...
    def hyper_optuna(self):
        if(self.estimator == self.nazwa_estymatora):
            def objective_cv(trial):
                ...
                for kwantyl in range(1, len(self.kwantyle)+1):
                    ...
                    param = {
                            ...
                            "C": trial.suggest_float("C", 1e-2, 1.0, log=True),
                            "random_state": 17
                    }
            ...
            tol_list = study_df.loc[:study_df.index[10], "params_tol"].values
            solver_list = study_df.loc[:study_df.index[10], "params_solver"].values
            C_list = study_df.loc[:study_df.index[10], "params_C"].values
            i = 0
            best_mean_score = 0
            while(i < len(tol_list)):
                scores = []
                for kwantyl in range(1, len(self.kwantyle)+1):
                    train = self.train_lista[kwantyl-1]
                    valid = self.valid_lista[kwantyl-1]
                    ...
                    for fold in range(1, self.number_of_folds+1):
                        ...
                        model = LogisticRegression(tol=tol_list[i], solver=solver_list[i], C=C_list[i], random_state=17)
            ...
            for kwantyl in range(1, len(self.kwantyle)+1):
                ...
                for fold in range(1, self.number_of_folds+1):
                    ...
                    estimator_final = LogisticRegression(tol=tol_list[best_set], solver=solver_list[best_set], C=C_list[best_set], random_state=17)
