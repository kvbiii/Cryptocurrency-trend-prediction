...
def plotting(results_łączne, kwartyl_łączne, results_base, name, nazwa_estymatora, results_pojedynczy=None, kwartyl_pojedynczy=None):
    ...
    layout = go.Layout(xaxis=dict(ticks='outside', tickvals=[len(results_łączne[0].flatten().tolist()), len(results_łączne[0].flatten().tolist())+len(results_łączne[1].flatten().tolist())], ticklen = 35, tickwidth=3, showticklabels=False, title="Podzbiór"), xaxis2=dict(ticks='outside', tickvals=[i+len(results_łączne[0])//2 for i in [0, len(results_łączne[0]), len(results_łączne[0]) + len(results_łączne[1])]], ticktext=["Fold 1", "Fold 2", "Fold 3"], tickangle=0,ticklen=0,tickwidth=0, showticklabels=True, overlaying="x"), yaxis=dict(nticks=10, tickprefix="$", title="Wartość portfela"))
